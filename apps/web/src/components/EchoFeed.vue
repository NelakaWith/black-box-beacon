<template>
  <div class="space-y-4">
    ...
    <TransitionGroup name="data-sync" tag="div" class="space-y-2">
      <div
        v-for="log in logs"
        :key="log.id"
      ...
      >
        <div class="flex flex-col text-right min-w-[80px]">
          <span class="text-beacon-muted font-bold"
            >[{{ formatTime(log.timestamp) }}]</span
          >
          <span class="text-[9px] opacity-50">ID_{{ log.id.slice(0, 4) }}</span>
        </div>
        <span
          class="uppercase tracking-tight leading-relaxed text-neutral-400"
          >{{ log.content }}</span
        >
      </div>
    </TransitionGroup>
    ...
    <div
      v-if="logs.length === 0"
      class="text-center py-12 text-gray-600 font-mono text-[10px] tracking-widest uppercase"
    >
